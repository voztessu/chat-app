...
app.get("/search", (req, res) => {
  ...
  // Câu truy vấn cơ bản
  let query = `
  SELECT DISTINCT m.id, m.sender_name, m.message, m.reply_to_id, 
         r.sender_name AS reply_to_username, r.message AS reply_to_content, 
         r.date_send AS reply_to_date_send, m.date_send
  FROM messages m
  LEFT JOIN messages r ON m.reply_to_id = r.id
...
//   const role = req.query.role;
//   const where = role == "kh" ? `sender_name != ? AND` : `sender_name = ? AND`;
//   const countQuery = `
...
app.get("/unread-count", (req, res) => {
  ...
  const where = role == "kh" ? `sender_name != ? AND` : `sender_name = ? AND`;
  const countQuery = `
        SELECT id, sender_name, message, date_send
        FROM messages
        WHERE ${where} is_replied = 0 AND reply_to_id IS NULL
    `;
  pool.query(countQuery, [senderName], (err, results) => {
    ...
    const unreadCount = results.length != 0 ? results : 0;
    res.json({ unread_count: unreadCount });
  });
});
app.get("/search-duplicated", (req, res) => {
  const key = req.query.key;
  const searchQuery = `
  SELECT DISTINCT m.id, m.sender_name, m.message, m.reply_to_id, 
         r.sender_name AS reply_to_username, r.message AS reply_to_content, 
  ...
  WHERE m.message LIKE '%${key}%'
    ...
    `;
  pool.query(searchQuery, [key], (err, results) => {
    if (err) {
      console.error("Error counting unread messages:", err);
      return res.status(500).json({ error: "Unable to count unread messages" });
    }
    const searchDuplicatedQuery = results.length != 0 ? results : 0;
    res.json({ search_duplicated: searchDuplicatedQuery });
  });
});
...
io.on("connection", (socket) => {
  socket.on("self-chat", (data) => {
    ...
    pool.query(
      ...
      (err, results) => {
        ...
        if (data.reply_to_id) {
          // Cập nhật tin nhắn gốc để đánh dấu đã trả lời
  ...
  socket.on("delete-chat", (data) => {
    ...
    pool.query(deleteQuery, [data.id], (err, results) => {
      if (err) {
        console.error("Error delete message into database:", err);
        socket.emit("db_error", { error: "Unable to delete message" });
        return;
      } else {
        io.emit("chat-deleted", data);
      }
    });
  });
});
